<template>
  <div class="container" >
    <Breadcrumb :items="['wxsys.title', 'wxsys.wxuser.title']" />
    <a-card class="general-card onelineCard" style="height: calc(100% - 50px);">
      <a-row style="margin-bottom: 10px">
        <a-col :span="16">
          <a-space>
            <a-input :style="{width:'120px'}"  v-model="formModel.name" placeholder="姓名" allow-clear />
      ...
      >
        ...
        <template #subscribe="{ record }">
          <a-tag :color="record.subscribe==1?'#00b42a':'#f53f3f'" >{{ record.subscribe==1?"关注":"取消"}}</a-tag>
        </template>
...
<script lang="ts" setup>
  ...
  import { getList,upStatus,del} from '@/api/wxsys/wxuser';
